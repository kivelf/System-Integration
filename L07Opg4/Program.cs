...
namespace L07Opg4
{
    ...
    public class Program
    {
        ...
        {
            // bruger Content-based Router (s.230)
            ...
            {
                if (airlineQueue.Label.Equals("L07" + ContentBasedRouter(SASjsonString))) 
                {
            ...
            foreach (var airlineQueue in airlineMessageQueues)
            {
                if (airlineQueue.Label.Equals("L07" + ContentBasedRouter(SWjsonString)))
                {
            ...
            foreach (var airlineQueue in airlineMessageQueues)
            {
                if (airlineQueue.Label.Equals("L07" + ContentBasedRouter(KLMjsonString)))
                {
        ...
        public static string ContentBasedRouter(string s)
        {
            if (s.Contains("SAS")) return "SAS";
            else if (s.Contains("SW")) return "SW";
            else if (s.Contains("KLM")) return "KLM";
            else return "unknown";
